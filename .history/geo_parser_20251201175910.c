...
int GeoParser_Ler(const char* caminho_geo, ListT formas) {
    ...
    while (fscanf(f, "%31s", cmd) == 1) {
        if (strcmp(cmd, "r") == 0) {
            ...
            if (fscanf(f, "%d %lf %lf %lf %lf %31s %31s", &id, &x, &y, &w, &h, corb, corp) >= 6) {
            ...
            if (fscanf(f, "%d %lf %lf %lf %31s %31s", &id, &x, &y, &r, corb, corp) >= 5) {
                PontoT center = Ponto_Criar(x,y);
                List_Inserir(formas, Segmento_Criar(id, Ponto_Criar(x-r,y), Ponto_Criar(x+r,y), corb));
            }
        } else {
            /* skip rest of line */
            char line[256];
