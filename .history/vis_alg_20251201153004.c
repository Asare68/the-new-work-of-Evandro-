...
// 2. Create Si and Sf sets by intersecting segments with the
initial ray r [17, 21].
 // 3. Activate segments in Si [21].
 SegsAtvsT segs_ativos = SegsAtvs_Criar(x_observador);

 // 4. Determine initial v0 and Biombo (Screen/Barrier) [17,
21]
 // v0 ← segAtivoMaisProx(x);
 PontoT biombo = /* ... initialization ... */ ; // Stores the
start point of the current visible sub-segment [23]

 // 5. Extract and Order Vertices
 ListT verticesLst = List_ExtrairVertices(S); // Extract
vertices from segments [11, 21]
 // Ordene verticesLst according to the established criteria
(Angle, Distance, Type) [5, 21].

 // 6. Vertex Processing (Planar Sweep) [11, 12, 21, 33]
 /*
 foreach v ∈ verticesLst do
 if v.tipo = START then
 // Logic for a start vertex (activation, updating V,
setting new biombo) [11, 34]
 if !encoberto(v) then
 r ← raio(x , v) ; s ← segAtivoMaisProx(v); y ←
interseccao(r , s) ;
 insira(V , [biombo, y ]) ; insira(V , [y , v ]) ;
